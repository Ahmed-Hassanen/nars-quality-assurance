...
exports.viewImprovementSuggestions = catchAsync(async (req, res, next) => {
  if (
    req.headers.authorization &&
    req.headers.authorization.startsWith("Bearer")
  ) {
  ...
  ////////////////////////////////////////////////////////
  const imporvementsuggestions = [];
  originalCourses.data.forEach((originalCourse) => {
    if (
      originalCourse.currentInstance &&
      originalCourse.currentInstance.imporvementSuggestions.length > 0
    ) {
      imporvementsuggestions.push(
        originalCourse.currentInstance.imporvementSuggestions
      );
    }
  });
  ////////////////////////

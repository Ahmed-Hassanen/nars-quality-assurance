...
exports.deleteAssignmentSolution =   catchAsync(async (req, res, next) => {
  ...
  exports.getAssignmentSolution = catchAsync(async (req, res, next) => {
    let query = AssignmentSolution.findById(req.params.id).populate({ path: "Assignment" });
...
if (!assignment) {
  ...
  const T = Date.now();
    ...
    res.download(path.resolve(`/${__dirname}/../public/assignmentSolutions/${assignmentSolution.solutionPath}`));
  });
